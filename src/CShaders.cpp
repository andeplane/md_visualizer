...
MDOpenGL* CShaderParent::MDOpenGLPointer;

CShaderParent::CShaderParent() {
  Shader = 0;
}
void CShaderParent::Initialize(string s) {
	if (MDOpenGLPointer==0) {
		throw string("Error in CShaderParent::Initialize - MDOpenGLPointer not set");
	}
	if (!Shader) {
		MDOpenGLPointer->shader_container.addshaderfrommemory(s,vert, frag);
		Shader = MDOpenGLPointer->shader_container.getshader(s);
...
void CShaderParent::ScreenToTexture(GLuint texture) {
  MDOpenGLPointer->buffer2texture(texture, MDOpenGLPointer->window_width, MDOpenGLPointer->window_height, MDOpenGL::NOMIPMAP);
}
...
void CBlurAndToonShader::Render() {
  if (!Shader)
    return;
  MDOpenGLPointer->SetOrthographicProjection();
  
  MDOpenGLPointer->push();
  glLoadIdentity();
  ...
  glVertex3f(0,0,0);
  glTexCoord2d(1,1);
  glVertex3f(MDOpenGLPointer->window_width,0,0);
  glTexCoord2d(1,0);
  glVertex3f(MDOpenGLPointer->window_width,MDOpenGLPointer->window_width,0);
  glTexCoord2d(0,0);
  glVertex3f(0,MDOpenGLPointer->window_width,0);
  glEnd();
  ...
  MDOpenGLPointer->ResetPerspectiveProjection(); 
...
void CBlurAndToonShader::Start() {
  ...
  Shader->sendUniform4f((char*)"camera_input", MDOpenGLPointer->cam->getXPos(),MDOpenGLPointer->cam->getYPos(),MDOpenGLPointer->cam->getZPos(), 1.0);
  if (Type!=BLUR) {
...
void CBlurAndToonShader::ScreenToTexture() {
  MDOpenGLPointer->buffer2texture(ScreenTexture, MDOpenGLPointer->window_width, MDOpenGLPointer->window_height, MDOpenGL::NOMIPMAP);
}
...
void CMathShader::Render() {
  ...
  glVertex3f(MDOpenGLPointer->window_width,0,0);
  glTexCoord2d(1,0);
  glVertex3f(MDOpenGLPointer->window_width,MDOpenGLPointer->window_height,0);
  glTexCoord2d(0,0);
  glVertex3f(0,MDOpenGLPointer->window_height,0);
  glEnd();
...
void CThinLensShader::Start() {
  ...
  Shader->sendUniform3f((char*)"center", center.x, center.y, center.z); //!< send vec3 to program
  Shader->sendUniform1f((char*)"lensStrength", lensStrength); //!< send vec3 to program
  Shader->sendUniform1f((char*)"Dd", Dd); //!< send vec3 to program
  Shader->sendUniform1f((char*)"type", type); //!< send vec3 to program
...
void C3DLensShader::Start() {
  ...
  //cout << center.x << "   " << center.y << endl;
  Shader->sendUniform1f((char*)"lensStrength", lensStrength); //!< send vec3 to program
  Shader->sendUniform3f((char*)"Delta", delta.x, delta.y, delta.z); //!< send vec3 to program
  Shader->sendUniform3f((char*)"Camera", camera.x, camera.y, camera.z); //!< send vec3 to program
  Shader->sendUniform3f((char*)"Min", min.x, min.y, min.z); //!< send vec3 to program
  Shader->sendUniform1f((char*)"type", type); //!< send vec3 to program
  Shader->sendUniform1f((char*)"Width", width); //!< send vec3 to program
  Shader->sendUniform1f((char*)"Scale", scale); //!< send vec3 to program
